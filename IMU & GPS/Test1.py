...
def apply_filter(ax, ay, az, ax_filtered, ay_filtered, az_filtered):
    """Apply a simple low-pass filter to the accelerometer data."""
    fc = 20
    fs = 100
    alpha = 2 * np.pi * fc / (fs + 2 * np.pi * fc)
...
def mpu_plot_test():
    ...
    while True:
        ...
        ax_filtered, ay_filtered, az_filtered = apply_filter(ax, ay, az, ax_filtered, ay_filtered, az_filtered)
        
        mpu_array[0:-1] = mpu_array[1:] # get rid of last point
        mpu_array[-1] = [ax_filtered,ay_filtered,az_filtered,wx,wy,wz,mx,my,mz] # update last point w/new data
