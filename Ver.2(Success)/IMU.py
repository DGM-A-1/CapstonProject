...
class IMU:   
    def __init__(self):
        self.accel_coeffs = [np.array([0.99814955, -0.13019077]), # 가속도계 보정 계수 (ax, bx)
                             np.array([0.99804448, 0.2620614]), # 가속도계 보정 계수 (ay, by)
                             np.array([0.98654515, 0.1548281])] # 가속도계 보정 계수 (az, bz)
    ...
    def initialize_pose(self):
        ...
        #print("Set your Car to North direction and press Enter")
        #input()
        #raw_yaw = self.compute_tilt_heading(mx, my, mz, acc_roll, acc_pitch)        
        #self.yaw_offset = raw_yaw  # 초기 yaw 오프셋 설정
        #self.yaw = 0.0
    ...
    """
    내부용: Roll(R), Pitch(P), Yaw(Y)(deg)로부터 Euler 각속도 변환 행렬 T 를 계산
    """
