# calculate_v_w_with_vis.py
# --------------------------------------------------------------------------
# 기존 calculate_v_w.py + matplotlib 시각화 통합 버전
# --------------------------------------------------------------------------
...
class VelocityController:
    """
    센서 데이터 수집·칼만필터·제어 연산만 수행.
    메인 스레드에서 시각화할 데이터를 리스트에 저장.
    """
    def __init__(self,
                 kp=1.0, ki=0.1, kd=0.05,
                 max_speed=0.25, desired_dist=2.0,
        ...
        # 시각화용 데이터 버퍼
        self.ins_traj = []   # INS(imu) 위치
        self.gps_traj = []   # GPS 위치
        self.kf_traj  = []   # Kalman 추정 위치
        self.history_t = []
        self.history_v = []
        self.history_w = []
    ...
    def _compute_angular(self, dx, dy, heading_deg):
        ...
        if diff > 180:
            diff -= 360
        angular_gain = 2.5
        return math.radians(angular_gain * diff)

    def start(self):
        logging.info("Starting VelocityController...")
        
        #추가) --- ROS 노드 초기화 & 퍼블리셔 생성 ---
    ...
    def _run_loop(self):
        ...
        while self.running and not rospy.is_shutdown():
            ...
            if dist <= self.desired_dist:
                ...
                self.w = 0.0
                print("Goal Reached")
                break   # while 루프 탈출하여 cleanup으로 이동
            
            
            los_angle_deg = math.degrees(math.atan2(dy, dx))
            heading_deg = imu_yaw
            angle_diff = (los_angle_deg - heading_deg + 180 )% 360 -180
            
            if abs(los_angle_deg) > 10:
                self.v = 0.0
                self.w = self._compute_angular(dx, dy, heading_deg)
            else:
