...
class IMU:   
    def __init__(self):
        self.accel_coeffs = [np.array([ 0.99712521,-0.13989452]), # 가속도계 보정 계수 (ax, bx)
                             np.array([0.99665928, 0.26531106]), # 가속도계 보정 계수 (ay, by)
                             np.array([0.99467918, 0.15954329])] # 가속도계 보정 계수 (az, bz)
    
        self.gyro_coeffs  = np.array([0.38753509521484375, -3.0309677124023438, 0.9673690795898438])
        self.mag_coeffs = np.array([53.98750305175781,  5.488014221191406, 34.05418395996094]) #[uT]
    ...
    def initialize_pose(self):
        ...
        # 2) 가속도계로 Roll Pitch 계산 [degrees] 2d mode 추정
    ...
    """
    바디 프레임 각속도 (p, q, r) [degree/s]를 받아
    Euler 각의 시간 미분 (φ̇, θ̇, ψ̇) [degree/s]를 반환
    """
    ...
    def _update(self):
        ...
        while self.running:
            ...
            rot_mat = self.get_rotation_matrix(self.yaw) # [ENU]
            g = np.array([0, 0, GRAVITY]) #[m/s²]
            a_world = rot_mat @ (np.array(a_cal).T * GRAVITY) #[m/s²]
            a_non_gravity_enu = a_world - g #[m/s²]

            # 추가) ENU -> NED 변환
            a_non_gravity = np.array([a_non_gravity_enu[1], a_non_gravity_enu[0], -a_non_gravity_enu[2]])
