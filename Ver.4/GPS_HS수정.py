...
# GPSreader 클래스를 정의하여 다른 파일에서 라이브러리처럼 사용할 수 있도록 함
class GPSReader:
    ...
    def start(self):
        ...
        self.origin = Point(35.8875834, 128.6117572) # 위도 경도값
        self.position_xy = (0.0, 0.0)  # 위도,경도값을 0,0으로 변환한 값 -> 직관적으로 이해하기 쉽게 하기 위해서
        self.running = True
        self.thread.start()
        return True
    ...
    def _read_loop(self):
        ...
        while self.running:
            try:
                ...
                lat = getattr(report, 'lat', None) # 위도
                lon = getattr(report, 'lon', None) # 경도
